...
export function Applications() {
  ...
  const { data: apps = [], isLoading } = useQuery<Application[]>({
    ...
    queryFn: () => getApplications(search || undefined).then((r) => r.data),
  });
    ...
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-2.5 font-medium text-gray-500 text-xs">Owner Team</th>
            ...
            <tbody>
              {apps.map((app) => (
                <tr key={app.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{app.owner_team ?? "—"}</td>
                  {canWrite && (
                    <td className="px-4 py-3 text-right flex gap-2 justify-end">
                      <button onClick={() => setEditing(app)} className="text-gray-300 hover:text-gray-700 transition-colors"><Pencil size={13} /></button>
                      <button onClick={() => setDeleteId(app.id)} className="text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={13} /></button>
                    </td>
                  )}
                </tr>
              ))}
              {apps.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400 text-sm">No applications found.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {(showForm || editing) && (
        <AppForm
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ["applications"] }); setShowForm(false); setEditing(null); }}
        />
      )}
      {deleteId !== null && (
        <ConfirmDialog message="Delete this application?" onConfirm={() => deleteMut.mutate(deleteId)} onCancel={() => setDeleteId(null)} />
      )}
    </div>
  );
}

function AppForm({ initial, onClose, onSaved }: { initial?: Application; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: initial?.name ?? "", description: initial?.description ?? "", owner_team: initial?.owner_team ?? "", notes: initial?.notes ?? "" });
  const mut = useMutation({
    mutationFn: (data: typeof form) => initial ? updateApplication(initial.id, data).then((r) => r.data) : createApplication(data).then((r) => r.data),
    onSuccess: onSaved,
  });
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm mx-4">
        <h2 className="text-base font-semibold text-gray-900 mb-4">{initial ? "Edit" : "Add"} Application</h2>
        <form onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} className="flex flex-col gap-3">
          {(["name", "description", "owner_team", "notes"] as const).map((k) => (
            <div key={k}>
              <label className="block text-xs font-medium text-gray-600 mb-1 capitalize">{k.replace("_", " ")}{k === "name" ? " *" : ""}</label>
